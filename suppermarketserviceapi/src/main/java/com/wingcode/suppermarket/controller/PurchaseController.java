...
public class PurchaseController {
	...
	@GetMapping("/purchases/itemscod/{itemCode}")
	public List<PurchaseItem> getByItemCode(@PathVariable(value = "itemCode") String itemCode) {
		return puriRepo.findByItemCode(itemCode);
	}
	...
	@GetMapping("/purchases/supplier/{param}/{paramValue}")
	public List<Purchase> getPurchaseBySupplierParam(@PathVariable(value = "param") String param, 
			@PathVariable(value = "paramValue") String paramValue) {
		switch (param) {
		case "co":
			return purRepo.findBySuplierCode(paramValue);
		case "nm":
			return purRepo.findBySupplierName(paramValue);
		default:
			return purRepo.findBySuplierCode(paramValue);
		}
	}
	
	@GetMapping("/purchases/dated/{date}")
	public List<Purchase> getPurchaseByDate(@PathVariable(value = "date") Date date) {
		return purRepo.findByPurchaseDate(date);
	}
	
	@GetMapping("/purchases/datepr/{param}/{paramValue}")
	public List<Purchase> getPurchaseByDateParam(@PathVariable(value = "param") String param, 
			@PathVariable(value = "paramValue") Integer paramValue) {
		switch (param) {
		case "dd":
			return purRepo.findByPurchaseDay(paramValue);
		case "mm":
			return purRepo.findByPurchaseMonth(paramValue);
		case "yy":
			return purRepo.findByPurchaseYear(paramValue);			
		default:
			return purRepo.findByPurchaseDay(paramValue);
		}
	}
	
	@GetMapping("/purchases/invoice/{invoiceNo}")
	public Purchase getPurchaseByInvoiceNo(@PathVariable(value = "invoiceNo") String invoiceNo) {
	...
	public Purchase updatePurchase(@PathVariable Long purchaseId, 
		...
		return purRepo.findById(purchaseId).map(p -> {
			...
			p.setOnUpdate(purchase.getOnUpdate());
			p.setUser(purchase.getUser());
			p.setPurchaseItems(purchase.getPurchaseItems());
			return purRepo.save(p);
