...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
	...
	@Query("select p from Purchase p where p.invoiceNo = ?1")
	Purchase findByInvoiceNo(String invoiceNo);
	
	@Query("select p from Purchase p where p.purchaseDate = ?1")
	List<Purchase> findByPurchaseDate(Date purchaseDate);
	
	@Query("select p from Purchase p where p.purchaseDate = ?1")
	List<Purchase> findByPurchaseDay(Integer purchaseDay);
	
	@Query("select p from Purchase p where p.purchaseMonth = ?1")
	List<Purchase> findByPurchaseMonth(Integer purchaseMonth);
	
	@Query("select p from Purchase p where p.purchaseYear = ?1")
